...
<script>
    $(document).ready(function() {
        ...
        // ==========================================================
        // LÓGICA CORREGIDA DEL BOTÓN DESCARGAR (LISTADO)
        // ==========================================================
        $('.print_invoice').click(function() {
            var id = $(this).attr('data-id');
            var code = $(this).attr('data-code');

            // Configuramos tamaño ventana
            var width = 900;
            var height = 900;
            var left = (screen.width - width) / 2;
            var top = (screen.height - height) / 2;

            // Abre ventana sin forzar impresión
            var nw = window.open("./invoice/print.php?id=" + id, "_blank", "width=" + width + ",height=" + height + ",top=" + top + ",left=" + left);

            if (nw) {
                nw.onload = function() {
                    // Solo cambiamos el título
                    nw.document.title = "FACTURA_" + code;
                    
                    // IMPORTANTE:
                    // Se eliminó nw.print() -> Para evitar la pantalla negra
                    // Se eliminó nw.close() -> Para que no se cierre sola
                };
                nw.focus();
            }
        });
        ...
        $('div.dataTables_filter input').attr('id', 'searchField');
        $('div.dataTables_filter input').attr('name', 'searchField'); 
